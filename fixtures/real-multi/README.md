# Real multi-tool fixture (structure-scrubbed)

- **Capture source:** real Pi Session JSONL from a local `pi-city` workspace run (change/verify with bash, write, edit, and read).
...
- **Scenario shape:** Session reconstruction with a multi-tool change/verify mix; not compatible with the bundled guided auth/multi lessons.
- **Regression coverage:** import + analysis of a real multi-tool Session tree, ensuring Evidence Explorer can open unmatched real runs without fabricated cinematic narration.
