...
## v0.4 Visual Prototype

The current visual branch replaces JSX-only hero buildings with independent GLB assets generated by `scripts/build_hero_models.py`. The harbor now includes a larger industrial fabric, cinematic sunset/cool-water lighting, fog, smoke, warehouses, tank farms, pipe racks, rail, dock lights and runtime-driven building overlays.

A build-free visual deployment shell lives in `site-visual-beta/`. It loads the GLBs directly in Three.js and can be deployed as a static site.

See `docs/visual-beta.md`.
