...
export function CityStoryHub({ campaign, onBeginMission, onOpenArchives }: { campaign: CityCampaignState; onBeginMission: (missionId: CityMissionId) => void; onOpenArchives: () => void }) {
  ...
  const backdrop = useMemo<FountainSessionState>(() => ({
    scenarioId: 'fountain-d-greybox', source: 'tutorial', phase: completed >= 3 ? 'complete' : 'arrival', facts: ['pressure-drop'], questionsAsked: [], completed: completed >= 3,
  }), [completed]);

  return <section className={`city-hub trust-${completed} chapter-${selectedChapter}`} aria-label="Pi City 心愿码头">
    ...
    <section className="trust-map trust-atlas" aria-label="城市信任图">
    ...
    <section className="wish-dock" aria-label={`第${selectedChapter}章居民心愿`}>
